...
class ERIGTO:
    ...
    def __init__(self, A, alpha, l_a, B, beta, l_b, C, gamma, l_c, D, delta, l_d):
        ...
        self.K_AB *= pow(2,0.5)*pow(np.pi, 5.0/4.0)/(self.p)
        self.K_CD *= pow(2,0.5)*pow(np.pi, 5.0/4.0)/(self.q)
    ...
    def auxiliary_integral_s(self, m):
        ...
        return pow((self.p + self.q), -0.5)*self.K_AB*self.K_CD*boys_kummer(m, T)
